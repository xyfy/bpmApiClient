...
> **注意：** `Auth:Authority` 与 `Auth:Audience` 用于 Host 侧 JWT Bearer 认证中间件（见第 2.2 节的 `AddAuthentication` / `AddJwtBearer` 注册）。  
...
// 使用 IHttpClientFactory 管理 HttpMessageHandler 生命周期，避免长寿命 HttpClient 导致的 DNS 过期问题
