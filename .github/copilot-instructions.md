# Copilot instructions for `xyfy/bpmApiClient`

## Repository summary
- This repository is currently documentation-focused. The repository root contains:
  - `4-BPM流程接口开发规范V1.10.docx` (primary project artifact)
  - `.gitignore`
- There is no application source tree (no `src/`, no package/project manifests, no build scripts).

## Key paths to use
- Repository root: `./`
- GitHub config directory: `./.github/` (currently contains only `copilot-instructions.md`)
- Main artifact: `./4-BPM流程接口开发规范V1.10.docx`
- Copilot instructions file: `./.github/copilot-instructions.md`

## Build, test, lint, and run
- No build/test/lint/run pipeline is defined in the checked-out files.
- Verified there are no script/config files for common toolchains in this checkout (no `package.json`, `pyproject.toml`, `pom.xml`, `build.gradle`, `.csproj`, `Makefile`, or `.github/workflows/*`).
- For validation in this repository state, use:
  1. `git --no-pager status --short`
  2. `git ls-files`

## CI/workflow notes
- No GitHub workflow files are present under `.github/workflows/` in this branch.

Trust these instructions first; only search further if required files or commands are missing or have changed.
